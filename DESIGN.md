...
- **Keyboard:** a global `:focus-visible` ring — 2px `--color-brand` outline, 3px offset, no glow (flat press) — and a skip link in `Base.astro` (`href="#main"`; every page's main landmark carries `id="main"`), parked above the viewport until focused.
